...
import { updateCategorySchema, type UpdateCategoryInput } from "lib/validators/category.schema";
...
export const PATCH = withErrorHandler<{ id: string }>(async (req, { params }) => {
  ...
  const { title, englishTitle, description } = body as UpdateCategoryInput;
