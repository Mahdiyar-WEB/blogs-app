import React from "react";

const PostsLoading = () => {
  return (
    <div className="animate-pulse grid grid-cols-12 gap-5">
      <div className="col-span-12 md:col-span-6 lg:col-span-4 border rounded-lg shadow-md ">
        <div className="w-full h-44 bg-secondary-300 rounded-t-lg overflow-hidden"></div>
        <div className="w-28 h-6 rounded-lg mt-5 bg-secondary-300 ms-2 me-auto"></div>
        <div className="flex justify-between items-center px-3 mt-5 ">
          <div className="flex gap-2 items-center">
            <span className="w-8 h-8 rounded-full bg-secondary-300"></span>
            <span className="w-28 h-6 rounded-lg bg-secondary-300"></span>
          </div>
          <div className="w-28 h-6 bg-secondary-300 rounded-lg"></div>
        </div>
        <div className="flex justify-between items-center px-3 mt-7 mb-4">
          <div className="h-8 w-24 rounded-md bg-secondary-300"></div>
          <div className="flex gap-2">
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
          </div>
        </div>
      </div>
      <div className="col-span-12 md:col-span-6 lg:col-span-4 border rounded-lg shadow-md ">
        <div className="w-full h-44 bg-secondary-300 rounded-t-lg overflow-hidden"></div>
        <div className="w-28 h-6 rounded-lg mt-5 bg-secondary-300 ms-2 me-auto"></div>
        <div className="flex justify-between items-center px-3 mt-5 ">
          <div className="flex gap-2 items-center">
            <span className="w-8 h-8 rounded-full bg-secondary-300"></span>
            <span className="w-28 h-6 rounded-lg bg-secondary-300"></span>
          </div>
          <div className="w-28 h-6 bg-secondary-300 rounded-lg"></div>
        </div>
        <div className="flex justify-between items-center px-3 mt-7 mb-4">
          <div className="h-8 w-24 rounded-md bg-secondary-300"></div>
          <div className="flex gap-2">
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
          </div>
        </div>
      </div>
      <div className="col-span-12 md:col-span-6 lg:col-span-4 border rounded-lg shadow-md ">
        <div className="w-full h-44 bg-secondary-300 rounded-t-lg overflow-hidden"></div>
        <div className="w-28 h-6 rounded-lg mt-5 bg-secondary-300 ms-2 me-auto"></div>
        <div className="flex justify-between items-center px-3 mt-5 ">
          <div className="flex gap-2 items-center">
            <span className="w-8 h-8 rounded-full bg-secondary-300"></span>
            <span className="w-28 h-6 rounded-lg bg-secondary-300"></span>
          </div>
          <div className="w-28 h-6 bg-secondary-300 rounded-lg"></div>
        </div>
        <div className="flex justify-between items-center px-3 mt-7 mb-4">
          <div className="h-8 w-24 rounded-md bg-secondary-300"></div>
          <div className="flex gap-2">
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
          </div>
        </div>
      </div>
      <div className="col-span-12 md:col-span-6 lg:col-span-4 border rounded-lg shadow-md ">
        <div className="w-full h-44 bg-secondary-300 rounded-t-lg overflow-hidden"></div>
        <div className="w-28 h-6 rounded-lg mt-5 bg-secondary-300 ms-2 me-auto"></div>
        <div className="flex justify-between items-center px-3 mt-5 ">
          <div className="flex gap-2 items-center">
            <span className="w-8 h-8 rounded-full bg-secondary-300"></span>
            <span className="w-28 h-6 rounded-lg bg-secondary-300"></span>
          </div>
          <div className="w-28 h-6 bg-secondary-300 rounded-lg"></div>
        </div>
        <div className="flex justify-between items-center px-3 mt-7 mb-4">
          <div className="h-8 w-24 rounded-md bg-secondary-300"></div>
          <div className="flex gap-2">
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
          </div>
        </div>
      </div>
      <div className="col-span-12 md:col-span-6 lg:col-span-4 border rounded-lg shadow-md ">
        <div className="w-full h-44 bg-secondary-300 rounded-t-lg overflow-hidden"></div>
        <div className="w-28 h-6 rounded-lg mt-5 bg-secondary-300 ms-2 me-auto"></div>
        <div className="flex justify-between items-center px-3 mt-5 ">
          <div className="flex gap-2 items-center">
            <span className="w-8 h-8 rounded-full bg-secondary-300"></span>
            <span className="w-28 h-6 rounded-lg bg-secondary-300"></span>
          </div>
          <div className="w-28 h-6 bg-secondary-300 rounded-lg"></div>
        </div>
        <div className="flex justify-between items-center px-3 mt-7 mb-4">
          <div className="h-8 w-24 rounded-md bg-secondary-300"></div>
          <div className="flex gap-2">
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
          </div>
        </div>
      </div>
      <div className="col-span-12 md:col-span-6 lg:col-span-4 border rounded-lg shadow-md ">
        <div className="w-full h-44 bg-secondary-300 rounded-t-lg overflow-hidden"></div>
        <div className="w-28 h-6 rounded-lg mt-5 bg-secondary-300 ms-2 me-auto"></div>
        <div className="flex justify-between items-center px-3 mt-5 ">
          <div className="flex gap-2 items-center">
            <span className="w-8 h-8 rounded-full bg-secondary-300"></span>
            <span className="w-28 h-6 rounded-lg bg-secondary-300"></span>
          </div>
          <div className="w-28 h-6 bg-secondary-300 rounded-lg"></div>
        </div>
        <div className="flex justify-between items-center px-3 mt-7 mb-4">
          <div className="h-8 w-24 rounded-md bg-secondary-300"></div>
          <div className="flex gap-2">
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
            <span className="w-8 h-8 bg-secondary-300 rounded-lg"></span>
          </div>
        </div>
      </div>
    </div>
  );
};

export default PostsLoading;
